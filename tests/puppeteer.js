...
describe("jobs-ejs puppeteer test", function () {
  ...
  describe("puppeteer job operations", function () {
    this.timeout(20000);
    it("clicks on jobs list link", async () => {
      ...
      const pageParts = pageHtml.split("<tr>");
      expect(pageParts.length).to.equal(21);
    });
    it("opens the add job form", async () => {
      const addButton = await page.waitForSelector("button ::-p-text(add job)");
      await addButton.click();
      await page.waitForNavigation();
      await page.waitForSelector("h2 ::-p-text(Job Form)");
      this.company = await page.waitForSelector('input[name="company"]');
      this.position = await page.waitForSelector('input[name="position"]');
      this.addJob = await page.waitForSelector("button ::-p-text(Add Job)");
    });
    it("fills job form and submits", async () => {
      const { expect } = await import("chai");
      await this.company.type("google");
      await this.position.type("fullstack");
